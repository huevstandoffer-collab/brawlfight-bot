...
PVE_COOLDOWN = 45      # сек между волнами
TOWER_COOLDOWN = 25   # сек между этажами башни
_pve_locks: set[int] = set()
_tower_locks: set[int] = set()
...
async def cmd_pve(message: Message):
    ...
    if uid in _pve_locks:
        await message.answer("⏳ Волны уже идут, подожди...")
        return
    now = time.time()
    last = float(user.get("last_pve") or 0)
    if now - last < PVE_COOLDOWN:
        left = int(PVE_COOLDOWN - (now - last))
        await message.answer(f"⏳ Волны: подожди ещё <b>{left}</b> сек.", parse_mode="HTML")
        return
    _pve_locks.add(uid)
    try:
        await _cmd_pve_inner(message, user)
    finally:
        _pve_locks.discard(uid)


async def _cmd_pve_inner(message: Message, user: dict):
...
async def cmd_tower(message: Message):
    ...
    if not user:
        ...
        return
    uid = message.from_user.id
    if uid in _tower_locks:
        await message.answer("⏳ Бой на этаже уже идёт...")
        return
    now = time.time()
    last = float(user.get("last_tower") or 0)
    if now - last < TOWER_COOLDOWN:
        left = int(TOWER_COOLDOWN - (now - last))
        await message.answer(f"⏳ Башня: подожди ещё <b>{left}</b> сек.", parse_mode="HTML")
        return
    _tower_locks.add(uid)
    try:
        await _cmd_tower_inner(message, user)
    finally:
        _tower_locks.discard(uid)


async def _cmd_tower_inner(message: Message, user: dict):
    owned = await get_user_brawlers(message.from_user.id)
    bid = user["selected_brawler"]
    if bid not in owned:
        bid = 1
    ...
    if win:
        ...
        await update_user(message.from_user.id, tower_floor=new_floor, last_tower=time.time())
        text = (
